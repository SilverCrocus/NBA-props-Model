...
class RealDataModelEvaluator:
    ...
    def __init__(self):
        ...
        if not self.game_logs_dir.exists():
            raise FileNotFoundError(
                f"Game logs directory not found: {
                    self.game_logs_dir}"
            )
    ...
    def engineer_features(self, df):
        ...
        df["PRA_trend"] = df.groupby("PLAYER_NAME")["PRA"].transform(
            ...
            .apply(lambda y: np.polyfit(range(len(y)), y, 1)[0] if len(y) > 1 else 0, raw=False)
        )
        ...
        print(
            f"✓ Created {len([c for c in df.columns if c.startswith(('PRA_', 'PTS_', 'REB_', 'AST_', 'MIN_'))])} features"  # noqa: E501
        )
    ...
    def prepare_datasets(self, df):
        ...
        return X_train, X_val, X_test, y_train, y_val, y_test, available_features  # noqa: E501
    ...
    def create_visualizations(self, y_test, test_pred, results):
        ...
        colors = [
            (
                "green"
                if m == min(results.keys(), key=lambda k: results[k]["mae"])
                else "steelblue"
            )  # noqa: E501
        ...
        for bar, mae in zip(bars, maes):
            width = bar.get_width()
            ax4.text(
                width, bar.get_y() + bar.get_height() / 2, f"{mae:.2f}", ha="left", va="center"
            )
        ...
        ax6.scatter(
            range(len(sample_actual)), sample_actual, color="red", alpha=0.5, s=20, label="Actual"
        )
        ax6.scatter(
            range(len(sample_pred)), sample_pred, color="blue", alpha=0.5, s=20, label="Predicted"
        )
    ...
    def save_results(self, results, features):
        ...
        if importance_data:
            ...
            importance_path = self.results_dir / "feature_importance_real_data.csv"  # noqa: E501
...
def main():
    ...
    try:
        ...
        # Prepare datasets
        X_train, X_val, X_test, y_train, y_val, y_test, features = evaluator.prepare_datasets(
            df
        )  # noqa: E501
