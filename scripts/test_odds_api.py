    ...
    if response.status_code == 200:
        ...
        if len(data) > 0:
            ...
            if game.get("bookmakers"):
                ...
                print(f"  Markets available: {len(bookmaker.get('markets', []))}")  # noqa: E501
            ...
            print("This is normal if NBA season hasn't started or is between games")  # noqa: E501
...
try:
    ...
    if response.status_code == 200:
        data = response.json()
        print("✅ Historical odds endpoint works!")
        print(f"Data returned: {type(data)}")
        print(f"Length: {len(data) if isinstance(data, list) else 'N/A'}")

        if data:
            print("\n✅ HISTORICAL DATA AVAILABLE!")
            print("We can backtest with real odds!")
    elif response.status_code == 401:
        print("❌ Historical odds require higher API plan")
        print("Current plan may not include historical data access")
    elif response.status_code == 404:
        print("❌ Historical endpoint not available")
