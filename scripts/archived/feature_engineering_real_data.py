...
class RealGameFeatureEngineer:
    ...
    def load_season_data(self, season="2023 - 24"):
        ...
        if not season_file.exists():
            raise FileNotFoundError(
                ...
                "Run: uv run scripts/fetch_all_game_logs.py"
            )
    ...
    def tier1_core_performance(self, df):
        ...
        for window in self.rolling_windows:
            ...
            # Advanced stats
            df[f"FG_PCT_L{window}"] = df.groupby("PLAYER_NAME")["FG_PCT"].transform(  # noqa: E501
                lambda x: x.shift(1).rolling(window, min_periods=1).mean()
            )
            df[f"FG3_PCT_L{window}"] = df.groupby("PLAYER_NAME")["FG3_PCT"].transform(  # noqa: E501
                lambda x: x.shift(1).rolling(window, min_periods=1).mean()
            )
        ...
        print(
            f"✓ Created {len([c for c in df.columns if '_L' in c or '_EMA' in c])} core features"
        )  # noqa: E501
    ...
    def tier2_contextual_modulators(self, df):
        ...
        df["games_played_pct"] = df["games_played"] / df.groupby("PLAYER_NAME")[
            "games_played"
        ].transform("max")
    ...
    def tier3_temporal_dynamics(self, df):
        ...
        for window in [5, 10]:
            ...
            df[f"PRA_trend_{window}"] = df.groupby("PLAYER_NAME")["PRA"].transform(  # noqa: E501
                lambda x: x.shift(1)
            ...
            df[f"MIN_trend_{window}"] = df.groupby("PLAYER_NAME")["MIN"].transform(  # noqa: E501
                lambda x: x.shift(1)
                .rolling(window, min_periods=2)
                .apply(
                    lambda y: np.polyfit(range(len(y)), y, 1)[0] if len(y) > 1 else 0, raw=False
        ...
        for window in [5, 10, 15]:
            df[f"PRA_std_{window}"] = df.groupby("PLAYER_NAME")["PRA"].transform(  # noqa: E501
                lambda x: x.shift(1).rolling(window, min_periods=2).std()
            )
            df[f"MIN_std_{window}"] = df.groupby("PLAYER_NAME")["MIN"].transform(  # noqa: E501
                lambda x: x.shift(1).rolling(window, min_periods=2).std()
            )
        ...
        df["above_avg"] = (
            ...
            .apply(
                lambda x: (x["PRA"] > x["PRA"].shift(1).rolling(20, min_periods=5).mean()).astype(
                    int
                )
            )
        ...
        df["meets_avg_L10"] = (
            df.groupby("PLAYER_NAME")
            .apply(lambda x: (x["PRA"] >= x["PRA_L10"] * 0.8).astype(int))
            .reset_index(level=0, drop=True)
        )

        df["consistency_score"] = df.groupby("PLAYER_NAME")[
            "meets_avg_L10"
        ].transform(  # noqa: E501
        ...
        df["PRA_vs_season_L5"] = df.groupby("PLAYER_NAME")["PRA_vs_season"].transform(  # noqa: E501
            lambda x: x.shift(1).rolling(5, min_periods=1).mean()
        )

        print("✓ Created temporal dynamics features")
    ...
    def create_training_dataset(self, season="2023 - 24"):
        ...
        print(
            f"Filtered to {
                len(active_players)} players with {min_games}+ games"
        )
        ...
        print("  Target: target_PRA (next game)")

        # Show feature categories
        print("\nFeature Categories:")
        print(f"  Rolling averages: {len([c for c in feature_cols if '_L' in c])}")  # noqa: E501
        print(f"  EWMA features: {len([c for c in feature_cols if '_EMA' in c])}")  # noqa: E501
        print(f"  Trend features: {len([c for c in feature_cols if '_trend' in c])}")  # noqa: E501
        print(
            f"  Volatility features: {len([c for c in feature_cols if '_std' in c or '_cv' in c])}"  # noqa: E501
        )
        print(
            f"  Context features: {
                len(
                    [
                        c for c in feature_cols if any(
                            x in c for x in [
                                'rest',
                                'home',
                                'away',
...
def main():
    ...
    try:
        ...
        __df = engineer.create_training_dataset("2023 - 24")  # noqa: F841
        ...
        print("\n✅ Next steps:")
        print("1. Run model evaluation: uv run scripts/evaluate_models_real_data.py")  # noqa: E501
        print(
            "2. The training data is in: data/processed/training_data_real_2023 - 24.csv"
        )  # noqa: E501
