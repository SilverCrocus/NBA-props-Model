...
class GameLogAnalyzer:
    ...
    def load_game_logs(self, season: str = None) -> pd.DataFrame:
        ...
        if season:
            ...
            if not file_path.exists():
                print(
                    f"Error: {file_path} not found. Run fetch_all_game_logs.py first."
                )  # noqa: E501
                return None
    ...
    def compare_with_synthetic(self):
        ...
        # Load synthetic data
        synthetic_path = self.data_dir / "processed" / "player_features_2023_24.csv"  # noqa: E501
        if synthetic_path.exists():
            ...
            print(
                f"  Range: [{synthetic_df['PRA_estimate'].min():.2f}, "
        ...
        if real_df is not None:
            ...
            print(f"  Avg PRA Std (within-player): {player_avg['std'].mean():.2f}")  # noqa: E501

            print("\n⚠️ KEY INSIGHT:")
            print("  Synthetic data is a SEASON AVERAGE")
            print("  Real data has GAME-BY-GAME VARIATION")
            print("  Real std dev is much higher due to game-to-game variance!")  # noqa: E501
    ...
    def analyze_variance_components(self):
        ...
        print(
            f"Between-Player Variance: {
                between_player_var:.2f} ({
                between_player_var /
                total_variance *
                100:.1f}%)"
        )
        print(
            f"Within-Player Variance: {
                within_player_var:.2f} ({
                within_player_var /
                total_variance *
                100:.1f}%)"
        )
    ...
    def demonstrate_realistic_prediction(self):
        ...
        print(f"  R²: {r2:.3f}")
        print(
            f"  MAPE: {(np.abs(df['PRA'] - df['PRA_L5']) / df['PRA']).mean() * 100:.1f}%"
        )  # noqa: E501
    ...
    def create_training_dataset(self, season: str = "2023 - 24"):
        ...
        feature_cols = ["PRA_L3", "PRA_L5", "PRA_L10", "PRA_trend", "days_rest", "MIN"]
