...
print(
    f"   Date range: {
        game_logs['GAME_DATE'].min()} to {
            game_logs['GAME_DATE'].max()}"
)
...
dataset = builder.build_complete_dataset(
    merge_ctg=True, min_minutes_per_game=10.0, min_games_played=5
)
...
if len(duplicates) > 0:
    print(
        f"⚠️  WARNING: Found {
            len(duplicates)} duplicate player-game combinations!"
    )
    print("\nExample duplicates:")
...
ctg_cols = [
    ...
    if any(x in col for x in ["USG%", "TS%", "AST%", "TOV%", "ORB%", "DRB%"])
]
...
print(
    f"  Date range: {
        dataset['GAME_DATE'].min()} to {
            dataset['GAME_DATE'].max()}"
)
