...
for player_id in tqdm(unique_players, desc="Adding enhanced features"):
    ...
    for i, (idx, row) in enumerate(player_games.iterrows()):
        ...
        if len(past_games) >= 3:
            ...
            train_df.loc[idx, "MIN_L3_mean"] = (
                last_3["MIN"].mean() if "MIN" in last_3.columns else 0
            )
...
Original Training Data:
  Rows: {len(train_df):,}
  Features: {len([c for c in train_df.columns if c not in ['PRA', 'PLAYER_NAME', 'PLAYER_ID', 'GAME_ID', 'GAME_DATE', 'SEASON']])}  # noqa: E501
