...
This script fetches historical player prop odds (PRA - Points + Rebounds + Assists)  # noqa: E501
...
if isinstance(remaining, str) and remaining != "Unknown":
    if float(remaining) < 100:
        ...
        print("  Historical data collection needs ~500 - 1000 requests")
...
while current_date <= SEASON_END:
    ...
    params = {"apiKey": API_KEY, "dateFrom": date_from, "dateTo": date_to}
...
for i, event in enumerate(all_events, 1):
    ...
    if i % 10 == 0:
        print(f"\nProgress: {i}/{len(all_events)} ({i / len(all_events) * 100:.1f}%)")  # noqa: E501

    # Fetch historical odds for this specific event
    # Historical endpoint syntax
    props_url = f"{BASE_URL}/historical/sports/basketball_nba/events/{event_id}/odds"  # noqa: E501
...
if parsed_props:
    ...
    print(
        f"  Date range: {
            df_props['event_date'].min()} to {
            df_props['event_date'].max()}"
    )
    ...
    print("⚠️  No props data to parse")
