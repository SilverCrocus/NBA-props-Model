...
if remaining < estimated_requests:
    print(
        f"⚠️  WARNING: May not have enough quota ({remaining} < {estimated_requests})"
    )  # noqa: E501
...
while current_date <= SEASON_END:
    ...
    try:
        ...
        if events_resp.status_code != 200:
            pbar.write(
                f"❌ {date_str}: Error fetching events ({
        ...
        pbar.write(
            f"📅 {date_str}: {
                len(events)} games (filtered from {
                len(all_events)} total)"
        )
        ...
        for event in events:
            ...
            odds_url = (
                f"{BASE_URL}/historical/sports/basketball_nba/events/{event_id}/odds"  # noqa: E501
            )
            odds_params = {
                "apiKey": API_KEY,
                "regions": "us",
                ...
                if odds_resp.status_code == 200:
                    ...
                    for bookmaker in bookmakers:
                        for market in bookmaker.get("markets", []):
                            if market["key"] == "player_points_rebounds_assists":  # noqa: E501
                                ...
                                for outcome in market.get("outcomes", []):
                                    ...
                                    if player not in outcomes_dict:
                                        outcomes_dict[player] = {
                                            ...
                                            "timestamp": odds_data.get(
                                                "timestamp", ""
                                            ),  # noqa: E501
                                        }

                                    if outcome.get("name") == "Over":
                                        outcomes_dict[player]["over_price"] = outcome.get(
                                            "price"
                                        )  # noqa: E501
                                    elif outcome.get("name") == "Under":
                                        outcomes_dict[player]["under_price"] = outcome.get(
                                            "price"
                                        )  # noqa: E501
                                ...
                                for prop in outcomes_dict.values():
                                    if (
                                        prop.get("line") is not None
                                    ):  # Only if line exists  # noqa: E501
                                        all_props.append(prop)
                                        props_count += 1

                    pbar.write(
                        f"  ✅ {event['away_team']} @ {event['home_team']}: {props_count} props"  # noqa: E501
                    )
                    ...
                    pbar.write(
                        f"  ⚠️  {
                            event['away_team']} @ {
                            event['home_team']}: No props ({
                            odds_resp.status_code})"
                    )
...
if all_props:
    ...
    print("\nSample props:")
    print(
        df[["player_name", "line", "over_price", "under_price", "bookmaker", "event_date"]].head(10)
    )
