...
Triple-check the edge threshold analysis by examining actual bet-by-bet performance.  # noqa: E501
...
predictions_df["under_wins"] = predictions_df["actual"] < predictions_df["line"]  # noqa: E501

# Determine if our bet would have won (based on our edge direction)
predictions_df["would_win"] = predictions_df.apply(
    lambda row: row["over_wins"] if row["edge"] > 0 else row["under_wins"], axis=1  # noqa: E501
)
...
for _, row in bucket_df.iterrows():
    print(
        f"{
            row['edge_range']:>12} | Total: {
            row['total_predictions']:4} | Wins: {
                row['wins']:4} | Win Rate: {
                    row['win_rate'] * 100:5.1f}%"
    )
print()
...
for _, row in bucket_df.iterrows():
    if row["over_predictions"] > 0:
        print(
            f"{
                row['edge_range']:>12} | Total: {
                row['over_predictions']:4} | Wins: {
                row['over_wins']:4} | Win Rate: {
                    row['over_win_rate'] * 100:5.1f}%"
        )
print()
...
for _, row in bucket_df.iterrows():
    if row["under_predictions"] > 0:
        print(
            f"{
                row['edge_range']:>12} | Total: {
                row['under_predictions']:4} | Wins: {
                row['under_wins']:4} | Win Rate: {
                    row['under_win_rate'] * 100:5.1f}%"
        )
print()
...
print("✅ Saved threshold analysis to data/results/edge_threshold_win_rates.csv")  # noqa: E501

# Save predictions with edge analysis
predictions_df.to_csv("data/results/predictions_with_edge_analysis.csv", index=False)
print(
    "✅ Saved detailed predictions to data/results/predictions_with_edge_analysis.csv"
)  # noqa: E501
