...
def walk_forward_train_and_validate(
    ...
    tracker.start_run(
        ...
        tags={
            "model_type": "xgboost",
            "validation_type": "walk_forward_leak_free_full",
            "train_season": train_season,
            "val_season": val_season,
            "description": "Leak-free walk-forward with FULL training dataset (all dates)",
        },
    )
    ...
    try:
        ...
        for path in game_logs_paths:
            if Path(path).exists():
                ...
                if "PRA" not in df.columns and all(
                    col in df.columns for col in ["PTS", "REB", "AST"]
                ):
        ...
        for pred_date in tqdm(
            train_dates, desc="Building training data (FULL)"
        ):  # ALL training dates
