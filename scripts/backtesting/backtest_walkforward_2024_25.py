"""
Backtest Walk-Forward Predictions for 2024-25

Uses predictions with NO temporal leakage to get true out-of-sample performance.
"""
...
print("\n" + "=" * 80)
print("2024-25 WALK-FORWARD BACKTEST RESULTS (NO TEMPORAL LEAKAGE)")
print("=" * 80)
...
print(
    f"Within ±3 pts: {(abs(merged['actual_pra'] - merged['predicted_pra']) <= 3).mean()*100:.1f}%"
)
print(
    f"Within ±5 pts: {(abs(merged['actual_pra'] - merged['predicted_pra']) <= 5).mean()*100:.1f}%"
)
...
if total_bets > 0:
    ...
    print(f"\n" + "=" * 80)
    print("FINAL SUMMARY - TRUE OUT-OF-SAMPLE VALIDATION")
    print("=" * 80)

    print(
        f"""
✅ 2024-25 WALK-FORWARD BACKTEST COMPLETE
...
🔧 Validation Method:
  ✅ Walk-forward validation (NO temporal leakage)
  ✅ Each prediction uses ONLY past games
  ✅ Deduplicated predictions (one per player-date)
  ✅ Line shopping across all bookmakers

Result: {'✅ PROFITABLE!' if total_profit > 0 else '❌ Not profitable'}

Comparison to Previous Results:
  2023-24 (corrected):     79.66% win rate, +61.98% ROI, 4.82 MAE
  2024-25 (with leakage):  99.35% win rate, +94.88% ROI, 1.51 MAE
  2024-25 (walk-forward):  {won_bets/total_bets*100:.2f}% win rate, {total_profit/total_wagered*100:+.2f}% ROI, {mae:.2f} MAE
...
Status: {'✅ CRUSHING ELITE BENCHMARKS' if total_profit/total_wagered*100 > 12 else '✅ EXCEEDING GOOD BENCHMARKS' if total_profit/total_wagered*100 > 5 else '⚠️  BELOW GOOD BENCHMARKS' if total_profit > 0 else '❌ UNPROFITABLE'}
"""
    )
