...
print(
    f"   Top feature: {
        model_dict['feature_importance'].iloc[0]['feature']} ({
            model_dict['feature_importance'].iloc[0]['importance'] *
        100:.1f}%)"
)
print()
...
print(
    f"   Date range: {
        df_historical['GAME_DATE'].min()} to {
...
print("   ✅ CTG data loaded")

# Merge CTG features
df_historical["CTG_SEASON"] = df_historical["SEASON"].apply(
...
for i, (idx, row) in enumerate(unique_player_seasons.iterrows(), 1):
    ...
    if i % 500 == 0:
        print(f"      Processed {i} / {len(unique_player_seasons)} player-seasons...")  # noqa: E501
...
df_historical = df_historical.merge(ctg_df, on=["PLAYER_NAME", "CTG_SEASON"], how="left")
...
for player_name in players_with_odds:
    ...
    if len(player_past) < 5:
        print(
            f"   ⚠️  Only {
                len(player_past)} games for {player_name}, skipping"
        )
        continue
    ...
    matchup = (
        player_past["MATCHUP"].iloc[-1] if "MATCHUP" in player_past.columns else ""
    )  # noqa: E501
    ...
    min_values = (
        player_past["MIN"].values if "MIN" in player_past.columns else np.array([])  # noqa: E501
    )
...
print(
    f"   Range: {
        df_pred['predicted_PRA'].min():.1f} to {
            df_pred['predicted_PRA'].max():.1f}"
)
...
print(
    f"   Missing Minutes_Projected: {
        df_pred['Minutes_Projected'].isna().sum()}"
)
print()

print("🏀 Top 10 Predictions:")
print(
    df_pred[["PLAYER_NAME", "TEAM", "predicted_PRA", "L5_mean_PRA", "Minutes_Projected"]]
    .head(10)
