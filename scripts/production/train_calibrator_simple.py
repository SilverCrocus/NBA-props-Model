...
print(
    f"   Date range: {
        df_2023['game_date'].min()} to {
            df_2023['game_date'].max()}"
)
print()
...
print(
    f"   Mean residual: {mean_residual_after:+.2f} pts (before: {mean_residual_before:+.2f})"
)  # noqa: E501
...
if mae_2024_after < mae_2024_before:
    print(
        f"   ✅ Calibration improved MAE by {
            mae_2024_before -
            mae_2024_after:.2f} pts"
    )
else:
    print("   ⚠️  Calibration did not improve MAE (this is normal for out-of-sample)")  # noqa: E501

if abs(mean_residual_2024_after) < abs(mean_residual_2024_before):
    print(
        f"   ✅ Calibration reduced bias by {
            abs(mean_residual_2024_before) -
            abs(mean_residual_2024_after):.2f} pts"
    )
else:
    print("   ⚠️  Calibration did not reduce bias")
