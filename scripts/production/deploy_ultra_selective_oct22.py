...
CALIBRATED_PREDICTIONS_PATH = "data/results/predictions_2025_10_22_CALIBRATED.csv"  # noqa: E501
ODDS_PATH = "data/upcoming/odds_2025_10_22.csv"
OUTPUT_PATH = "data/results/betting_recommendations_oct22_2025_ULTRA_SELECTIVE.csv"  # noqa: E501
...
df_matched = df_pred.merge(df_odds, left_on="PLAYER_NAME", right_on="player_name", how="inner")
...
if len(ultra_selective) > 0:
    ...
    for i, (_, row) in enumerate(ultra_selective.head(10).iterrows(), 1):
        ...
        print(
            f"       [Edge: {
                row['score_edge']:.2f} | Conf: {
                row['score_confidence']:.2f} | Context: {
                row['score_context']:.2f} | Consistency: {
                    row['score_consistency']:.2f}]"
        )
        print()
else:
...
print(
    f"   Total wager: ${len(ultra_selective) *
                          50}-${len(ultra_selective) *
                                100}"
)
print(
    "   Expected wins: {:.1f} out of {}".format(len(ultra_selective) * 0.637, len(ultra_selective))
)
print()
