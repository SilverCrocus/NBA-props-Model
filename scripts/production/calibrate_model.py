...
Research shows calibration is MORE important than accuracy for betting profitability.  # noqa: E501
...
# Load validation predictions (generated by
# generate_val_predictions_2023_24.py)
...
print(
    f"   Prediction range: {
        val_predictions.min():.1f} to {
            val_predictions.max():.1f}"
)
...
print(
    f"   Raw probability range: {
        calib_df['prob_over_raw'].min():.3f} to {
            calib_df['prob_over_raw'].max():.3f}"
)
...
print(
    f"   Calibrated probability range: {
        calib_df['prob_over_calibrated'].min():.3f} to {
            calib_df['prob_over_calibrated'].max():.3f}"
)
...
print(
    f"    Brier score: {
        brier_calibrated:.4f} ({
            '✅ BETTER' if brier_calibrated < brier_raw else '❌ WORSE'})"
)
print(
    f"    Log loss: {
        log_loss_calibrated:.4f} ({
            '✅ BETTER' if log_loss_calibrated < log_loss_raw else '❌ WORSE'})"
)
...
binned_raw = (
    calib_df.groupby("prob_bin")
    .agg(
        {"actual_over": ["mean", "count"], "prob_over_raw": "mean", "prob_over_calibrated": "mean"}
    )
    .reset_index()
)
...
# Raw model
plt.scatter(
    binned_raw["predicted_prob_raw"],
    binned_raw["empirical_freq"],
    s=binned_raw["count"],
    alpha=0.6,
    label="Raw model",
    color="red",
...
# Calibrated model
plt.scatter(
    binned_raw["predicted_prob_calibrated"],
    binned_raw["empirical_freq"],
    s=binned_raw["count"],
    alpha=0.6,
...
print("\nUsage in Betting:")
print("  1. Load calibrated model")
print("  2. Predict PRA for upcoming game")
print("  3. Convert to P(over) using logistic transform")
print("  4. Apply calibrator: P_calibrated = calibrator.predict(P_raw)")
print("  5. Calculate edge: P_calibrated - implied_probability")
print("  6. Bet if edge >= 5%")

print("\nResearch-backed benefit:")
print("  Calibrated models can improve ROI by 30 - 70% vs raw predictions")
print("  (Walsh & Joshi 2023: +34.69% ROI calibrated vs -35.17% raw)")
