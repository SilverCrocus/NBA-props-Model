...
BETTING_OUTPUT_PATH = "data/results/backtest_2024_25_ULTRA_SELECTIVE_betting.csv"  # noqa: E501
...
def calculate_quality_score(row):
    ...
    __l5_mean = row.get("L5_mean_PRA", pred_pra)  # noqa: F841
...
print(
    f"   Reduction: {
        len(base_filtered):,} → {
            len(ultra_selective):,} ({
                len(ultra_selective) / len(base_filtered) * 100:.1f}%)"
)
print()
...
ultra_selective["bet_correct"] = (
    (ultra_selective["bet_type"] == "OVER")
    & (ultra_selective["actual_PRA"] > ultra_selective["line"])
) | (
    (ultra_selective["bet_type"] == "UNDER")
    & (ultra_selective["actual_PRA"] < ultra_selective["line"])
)
...
for i in range(len(score_bins) - 1):
    ...
    if len(bin_data) > 0:
        ...
        print(
            f"   Quality {score_bins[i]:.2f}-{score_bins[i + 1]:.2f}: {len(bin_data):4d} bets, {bin_win_rate * 100:.1f}% win rate"  # noqa: E501
        )
...
print()
print("   Phase 1 (5 - 7 pt edges):")
print("     Bets: 2,489")
print("     Win rate: 52.03%")
print("     ROI: -0.67%")
print()
print("   Ultra-Selective (quality ≥ 0.75):")
...
if total_bets >= 300 and total_bets <= 500:
    print("   ✅ Volume target met: 300 - 500 bets/season")
else:
    print(f"   ⚠️  Volume: {total_bets} (target: 300 - 500)")

if win_rate >= 0.56:
    print("   ✅ Win rate target met: ≥56%")
elif win_rate >= 0.54:
