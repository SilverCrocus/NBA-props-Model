...
def analyze_feature_inventory():
    ...
    print("\n### 7. ON/OFF - EFFICIENCY & FOUR FACTORS")
    file_path = season_dir / "on_of" / "efficiency_four_factors" / "efficiency_four_factors.csv"
    if file_path.exists():
        df = pd.read_csv(file_path, nrows=3)
        features = [col for col in df.columns if col not in ["Player", "Age", "Team", "Pos"]]
    ...
    file_path = season_dir / "on_of" / "team_shooting_frequency" / "team_shooting_frequency.csv"
    if file_path.exists():
        df = pd.read_csv(file_path, nrows=3)
        features = [col for col in df.columns if col not in ["Player", "Age", "Team", "Pos"]]
    ...
    # Game Log Features
    print("\n### Game Log Features:")
    game_logs_file = base_dir / "data" / "game_logs" / "all_game_logs_combined.csv"  # noqa: E501
