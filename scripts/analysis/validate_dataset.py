...
if len(sample_player) >= 10:
    ...
    if abs(l5_mean - manual_l5) < 0.01:
        print("✅ PASSED: Rolling average correctly excludes current game")
    else:
        print("❌ FAILED: Rolling average may include current game")
...
if len(sample_player) >= 3:
    ...
    if abs(expected_rest - actual_rest) < 0.1:
        print("✅ PASSED: Days rest calculated correctly")
    else:
        print("❌ FAILED: Days rest calculation error")
...
feature_categories = {
    ...
    "Rolling": [
        col for col in df.columns if "_L5_" in col or "_L10_" in col or "_L20_" in col
    ],  # noqa: E501
...
print(
    f"\nTarget variable (PRA) missing: {
        df['PRA'].isnull().sum():,} ({
            df['PRA'].isnull().sum() / len(df) * 100:.2f}%)"
)
