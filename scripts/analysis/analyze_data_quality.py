...
class DataQualityAnalyzer:
    ...
    def __init__(self, base_dir: str):
        ...
        self.player_data_dir = (
            self.base_dir / "data" / "ctg_data_organized" / "players"
        )  # noqa: E501
            ...
            if split_dir.exists():
                ...
                for category_dir in sorted(split_dir.iterdir()):
                    if category_dir.is_dir():
                        ...
                        if csv_files:
                            print(
                                f"    - {category_dir.name}: {len(csv_files)} file(s)"
                            )  # noqa: E501
    ...
    def analyze_data_quality(self):
        ...
        for csv_file in csv_files:
            try:
                ...
                quality_metrics["data_issues"].append(
                    {"file": csv_file.name, "issue": f"Read error: {str(e)}"}
                )

        print(f"\nFiles analyzed: {quality_metrics['files_analyzed']}")
        print(f"Total rows sampled: {quality_metrics['total_rows']:,}")
        print(
            f"Average columns per file: {
                quality_metrics['total_columns'] //
                quality_metrics['files_analyzed']}"
        )
    ...
    def analyze_team_data(self):
        ...
        self.results["team_data"] = {"teams": len(team_dirs), "total_files": total_team_files}
    ...
    def generate_summary_report(self):
        ...
        if progress.get("missing", 0) > 0:
            ...
            print("   - Run: uv run ctg_robust_scraper.py")

        if len(quality.get("data_issues", [])) > 0:
            print("\n2. Address data quality issues")
            print("   - Review and clean problematic files")

        print("\n3. Next steps for modeling:")
        print("   - Develop feature engineering pipeline")
        print("   - Merge player stats with game logs")
        print("   - Create PRA (Points + Rebounds + Assists) target variable")
        print("   - Build train/test split by season")
