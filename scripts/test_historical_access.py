...
# Test with 2023 - 24 season opener (October 24, 2023) - we know this game
# happened
...
try:
    ...
    if response.status_code == 200:
        ...
        if len(events) > 0:
            ...
            if not event:
                print("⚠️  Unable to parse event data")
                print("=" * 80)
                exit(1)
            event_id = event["id"]
            print("\nEvent details:")
            ...
            print(
                f"Requests remaining: {
                    hist_response.headers.get('x-requests-remaining')}"
            )
            ...
            if hist_response.status_code == 200:
                ...
                print("Historical data requires upgraded API plan")
                print(f"Response: {hist_response.text}")

            elif hist_response.status_code == 404:
                print("\n⚠️  NOT FOUND (404)")
                print("Historical data may not be available for this date")
                print(f"Response: {hist_response.text}")
