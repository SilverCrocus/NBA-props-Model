"""
Collect 2024 - 25 NBA Season Historical Betting Odds

Fetches PRA betting lines for the entire 2024 - 25 season (Oct 2024 - June 2025).  # noqa: E501
Uses the same parallel, filtered approach as 2023 - 24 collection.
"""
...
SEASON_END = datetime(2025, 6, 17)
OUTPUT_DIR = Path("data/historical_odds/2024 - 25")
...
print("=" * 80)
print("2024 - 25 NBA SEASON - HISTORICAL PRA ODDS COLLECTOR")
print("=" * 80)
print("\nSettings:")
...
def fetch_event_odds(event, date_str):
    ...
    odds_url = f"{BASE_URL}/historical/sports/basketball_nba/events/{event_id}/odds"  # noqa: E501
    odds_params = {
        "apiKey": API_KEY,
        "regions": "us",
                        ...
                        for outcome in market.get("outcomes", []):
                            ...
                            if player not in outcomes_dict:
                                outcomes_dict[player] = {
                                    ...
                                    "timestamp": odds_data.get("timestamp", ""),  # noqa: E501
                                }

                            if outcome.get("name") == "Over":
                                outcomes_dict[player]["over_price"] = outcome.get(  # noqa: E501
                                    "price"
                                )
                            elif outcome.get("name") == "Under":
                                outcomes_dict[player]["under_price"] = outcome.get(  # noqa: E501
                                    "price"
                                )
...
with tqdm(total=total_days, desc="Collecting", unit="day") as pbar:
    ...
    while current_date <= SEASON_END:
        ...
        events_params = {"apiKey": API_KEY, "date": current_date.strftime("%Y-%m-%dT12:00:00Z")}
        ...
        try:
            ...
            all_events = (
                events_data.get("data", []) if isinstance(events_data, dict) else events_data
            )
            ...
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                future_to_event = {
                    executor.submit(fetch_event_odds, event, date_str): event for event in events
                }
