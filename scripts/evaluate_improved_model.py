...
class ImprovedNBAPropsModel:
    ...
    def load_data(self, data_path: str = None) -> pd.DataFrame:
        ...
        if data_path is None:
            data_path = (
                Path(__file__).parent.parent / "data" / "game_logs" / "all_game_logs_combined.csv"
            )
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["usage_L5"] = (
            ...
            .apply(
                lambda x: (
                    x["FGA"].shift(1).rolling(5, min_periods=1).mean()
    ...
    def prepare_data(self, df: pd.DataFrame) -> tuple:
        ...
        if "scoring_vs_de" in df.columns:
            feature_cols.append("scoring_vs_de")
    ...
    def train_and_evaluate(self, df: pd.DataFrame, feature_cols: list) -> dict:
        ...
        for name, model in models.items():
            ...
            print(
                f"  MAE: {
                    cv_results['summary']['mae_mean']:.2f} ± {
                    cv_results['summary']['mae_std']:.2f}"
            )
            print(
                f"  RMSE: {
                    cv_results['summary']['rmse_mean']:.2f} ± {
                    cv_results['summary']['rmse_std']:.2f}"
            )
            print(
                f"  R²: {
                    cv_results['summary']['r2_mean']:.3f} ± {
                    cv_results['summary']['r2_std']:.3f}"
            )
        ...
        wf_results = self.validator.walk_forward_validation(
            best_model, X, y, dates, initial_train_size=10000, step_size=1000
        )
        ...
        print(
            f"Date range: {
                wf_results['date_range'][0].date()} to {
                wf_results['date_range'][1].date()}"
        )

        # Feature importance
        best_model.fit(X, y)
        feature_importance = pd.DataFrame(
            {"feature": feature_cols, "importance": best_model.feature_importances_}  # noqa: E501
...
def main():
    ...
    __results = model.train_and_evaluate(df_clean, feature_cols)  # noqa: F841
