...
class NBAPropsAnalyzer:
    ...
    def load_data(self):
        ...
        # Define file paths
        base_path = (
            self.data_path / "ctg_data_organized" / "players" / "2023 - 24" / "regular_season"
        )
        ...
        try:
            ...
            self.player_df = self.player_df.merge(
                defense[defense_cols], on=["Player", "Team"], how="left"
            )

            # Add shooting stats
            shooting_cols = ["Player", "Team", "eFG%", "2P%", "3P%", "FT%"]
            self.player_df = self.player_df.merge(
                shooting[shooting_cols], on=["Player", "Team"], how="left"
            )

            # Add foul drawing
            foul_cols = ["Player", "Team", "SFLD%", "FFLD%", "AND1%"]
            self.player_df = self.player_df.merge(
                fouls[foul_cols], on=["Player", "Team"], how="left"
            )

            print(
                f"✓ Loaded {len(self.player_df)} players with {len(self.player_df.columns)} features"  # noqa: E501
            )
    ...
    def create_pra_estimates(self):
        ...
        if all(col in self.player_df.columns for col in ["MIN", "Usage", "PSA"]):
            points = (
                self.player_df["MIN"] * (self.player_df["Usage"] / 100) * self.player_df["PSA"] * 2
            )
        else:
            points = 15  # Default
        ...
        if all(col in self.player_df.columns for col in ["MIN", "fgDR%", "fgOR%"]):
            rebounds = (
                self.player_df["MIN"]
                * ((self.player_df["fgDR%"] + self.player_df["fgOR%"]) / 100)
                * 4
            )
        else:
            rebounds = 5  # Default
        ...
        print(
            f"✓ Created PRA estimates (mean: {
                self.player_df['PRA_estimate'].mean():.1f})"
        )
    ...
    def calculate_volatility_scores(self):
        ...
        if volatility_factors:
            ...
            print("✓ Calculated volatility scores")

        return (
            self.player_df["Volatility_Score"]
            if "Volatility_Score" in self.player_df.columns
            else None
        )
    ...
    def create_risk_reward_matrix(self):
        ...
        conditions = [
            (qualified["PRA_estimate"] >= median_pra)
            & (qualified["Volatility_Score"] < median_volatility),
            (qualified["PRA_estimate"] >= median_pra)
            & (qualified["Volatility_Score"] >= median_volatility),
            (qualified["PRA_estimate"] < median_pra)
            & (qualified["Volatility_Score"] < median_volatility),
        ...
        print("✓ Created risk-reward categories:")
    ...
    def identify_player_clusters(self, n_clusters=6):
        ...
        print("✓ Identified player archetypes:")
    ...
    def visualize_insights(self):
        ...
        for category in categories:
            if category in colors:
                data = self.risk_reward[
                    self.risk_reward["Betting_Category"] == category
                ]  # noqa: E501
                ax1.scatter(
        ...
        ax1.axhline(
            y=self.risk_reward["PRA_estimate"].median(), color="gray", linestyle="--", alpha=0.5
        )
        ax1.axvline(
            x=self.risk_reward["Volatility_Score"].median(), color="gray", linestyle="--", alpha=0.5
        )
        ...
        ax2.set_yticklabels(
            [p.split()[-1] for p in top_pra["Player"].values]
        )  # Last name only  # noqa: E501
        ...
        if "Volatility_Rating" in self.risk_reward.columns:
            volatility_counts = self.risk_reward["Volatility_Rating"].value_counts()  # noqa: E501
            ax3.pie(
                volatility_counts.values,
                labels=volatility_counts.index,
                autopct="%1.1f%%",
        ...
        ax4 = axes[1, 1]
        ax4.scatter(
            self.risk_reward["Usage"],
            self.risk_reward["PRA_estimate"],
            c=self.risk_reward["MIN"],
            cmap="viridis",
            alpha=0.6,
        )
    ...
    def run_complete_analysis(self):
        ...
        if self.player_df is not None:
            ...
            # Identify clusters
            __clusters = self.identify_player_clusters()  # noqa: F841
            ...
            print(
                f"✓ Saved betting sheet to {
                    output_path /
                    'betting_recommendations.csv'}"
            )
            print(
                f"✓ Saved risk-reward analysis to {output_path / 'risk_reward_analysis.csv'}"
            )  # noqa: E501
...
def main():
    ...
    if betting_sheet is not None:
        ...
        print(
            f"- Premium plays: {(risk_reward['Betting_Category'] == 'Premium Plays').sum()}"
        )  # noqa: E501
        print(
            f"- High risk/reward: {(risk_reward['Betting_Category'] == 'High Risk/Reward').sum()}"  # noqa: E501
        )
    else:
