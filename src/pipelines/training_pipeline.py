...
class NBAPropsPipeline:
    ...
    def __init__(
        ...
        self.config = {
            ...
            "features": {"tier1_core": [], "tier2_contextual": [], "tier3_temporal": []},
            "selected_features": [],
            "model_metrics": {},
        }

    def run_full_pipeline(
        self, seasons: List[str], target_season: str = None, save_artifacts: bool = True
    ) -> Dict:
        """
        ...
        Args:
            ...
            target_season: Season to use for testing (if None, uses train/test split)  # noqa: E501
        ...
        logger.info(
            f"Loaded {
                len(combined_data)} samples from {
                len(seasons)} seasons"
        )
        ...
        high_var_features = self.feature_selector.variance_threshold(
            X_train_filtered, threshold=0.01
        )
        X_train_filtered = X_train_filtered[high_var_features]
        X_test_filtered = X_test_filtered[high_var_features]

        logger.info(
            f"After filtering: {len(X_train_filtered.columns)} features remain"
        )  # noqa: E501
        ...
        # LASSO
        lasso_result = self.feature_selector.lasso_selection(X_train_filtered, y_train, alpha=0.01)
        ...
        # ========== Step 8: Save Results ==========
        if save_artifacts:
            self.save_pipeline_artifacts(engineered_features, final_features, results)
    ...
    def categorize_features(self, features: List[str]):
        ...
        for feature in features:
            ...
            # Check Tier 3 first (temporal features often contain tier 1
            # keywords)
        ...
        logger.info(
            f"Tier 1 (Core): {len(self.config['features']['tier1_core'])} features"
        )  # noqa: E501
        logger.info(
            f"Tier 2 (Contextual): {len(self.config['features']['tier2_contextual'])} features"
        )  # noqa: E501
        logger.info(
            f"Tier 3 (Temporal): {len(self.config['features']['tier3_temporal'])} features"
        )  # noqa: E501

    def save_pipeline_artifacts(
        self, features_df: pd.DataFrame, selected_features: List[str], results: Dict
    ):
        """
        ...
        with open(config_path, "w") as f:
            json.dump(
                {"config": self.config, "results": json_results, "timestamp": timestamp},
                f,
                indent=2,
                default=str,
            )
...
def main():
    ...
    # Save report
    report_path = Path(
        "/Users/diyagamah/Documents/nba_props_model/feature_engineering_report.txt"
    )  # noqa: E501
