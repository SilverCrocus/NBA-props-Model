...
class CategoricalFeatureHandler:
        ...
        if model_type == "tree":
            ...
            categorical_transformer = Pipeline(
                [
                    ...
                    )  # Drop first to avoid multicollinearity  # noqa: E501
                ]
            )
    ...
    def fit_transform(self, X: pd.DataFrame, model_type: str = "tree") -> np.ndarray:
        """
    ...
    def get_feature_names(self, preprocessor: ColumnTransformer, model_type: str) -> List[str]:
        """
        Get feature names after transformation.
    ...
    def encode_for_multiple_models(self, X: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        """
        ...
        # Store preprocessors
        self.preprocessors = {"tree": tree_preprocessor, "linear": linear_preprocessor}
...
def prepare_features_for_modeling(
...
) -> Tuple[Dict, pd.Series, CategoricalFeatureHandler]:  # noqa: E501  # noqa: E501  # noqa: E501
    """
    ...
    handler = CategoricalFeatureHandler(
        categorical_columns=categorical_cols, numeric_columns=numeric_cols
    )
...
def get_feature_importance_with_categories(
    model, feature_names: List[str], categorical_mappings: Optional[Dict] = None
) -> pd.DataFrame:
    """
    ...
    if categorical_mappings:
        ...
        for feat in importance_df["feature"]:
            ...
            for cat_col in categorical_mappings.keys():
                if feat.startswith(f"{cat_col}_"):
                    ...
                    aggregated[cat_col] += importance_df[
                        importance_df["feature"] == feat  # noqa: E501
                    ]["importance"].values[
                        0
                    ]  # noqa: E501
                    is_encoded = True
            ...
            if not is_encoded:
                # Regular feature
                aggregated[feat] = importance_df[importance_df["feature"] == feat][
                    "importance"
                ].values[
                    0
                ]  # noqa: E501
        ...
        importance_df = pd.DataFrame(list(aggregated.items()), columns=["feature", "importance"])

    return importance_df.sort_values("importance", ascending=False)


# Example usage for different model types
def get_model_specific_features(
    df: pd.DataFrame, model_name: str, categorical_cols: List[str], numeric_cols: List[str]
) -> Tuple[pd.DataFrame, ColumnTransformer]:  # noqa: E501
    """
