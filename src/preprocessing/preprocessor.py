...
Handles data loading, merging, cleaning, and preparation for feature engineering  # noqa: E501
"""
...
class NBADataPreprocessor:
    ...
    def __init__(
        ...
        self.stat_categories = {
            ...
            "on_of": [
                ...
                "team_shooting_frequency",
                "opponent_shooting_frequency",
            ],
            "context": ["team_transition", "opponent_transition", "team_halfcourt_putbacks"],
        }

    def load_season_data(
        self, season: str, data_type: str = "regular_season"
        ...
        offensive_path = season_path / "offensive_overview" / "offensive_overview.csv"  # noqa: E501
        if offensive_path.exists():
            data_dict["offensive_overview"] = pd.read_csv(offensive_path)
            logger.info(
                f"Loaded offensive overview: {len(data_dict['offensive_overview'])} rows"
            )  # noqa: E501

        # Load defense/rebounding
        defense_path = season_path / "defense_rebounding" / "defense_rebounding.csv"  # noqa: E501
        if defense_path.exists():
            data_dict["defense_rebounding"] = pd.read_csv(defense_path)
            logger.info(
                f"Loaded defense/rebounding: {len(data_dict['defense_rebounding'])} rows"
            )  # noqa: E501

        # Load shooting data
        shooting_freq_path = (
            season_path / "shooting_frequency" / "shooting_frequency.csv"
        )  # noqa: E501
        if shooting_freq_path.exists():
            data_dict["shooting_frequency"] = pd.read_csv(shooting_freq_path)

        shooting_acc_path = (
            season_path / "shooting_accuracy" / "shooting_accuracy.csv"
        )  # noqa: E501
        if shooting_acc_path.exists():
            data_dict["shooting_accuracy"] = pd.read_csv(shooting_acc_path)

        # Load on/off data
        on_off_path = season_path / "on_of"
    ...
    def merge_player_data(self, data_dict: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        for key, df in data_dict.items():
            ...
            cols_to_add = [
                col for col in df.columns if col not in merged.columns or col in merge_keys
            ]  # noqa: E501
            ...
            if len(cols_to_add) > len(merge_keys):
                try:
                    ...
                    merged = merged.merge(
                        df[cols_to_add], on=merge_keys, how="left", suffixes=("", f"_{key}")
                    )
                except BaseException:
                    # Try merging on Player only if Team causes issues
                    merged = merged.merge(
                        df[cols_to_add], on="Player", how="left", suffixes=("", f"_{key}")
                    )
    ...
    def load_team_data(self, teams: List[str] = None) -> pd.DataFrame:
        ...
        for team_dir in team_dirs:
            ...
            efficiency_file = (
                team_dir / "team_efficiency_and_four_factors_all_seasons.csv"
            )  # noqa: E501
    ...
    def handle_multi_team_players(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(multi_team_players) > 0:
            logger.info(
                f"Found {
                    len(multi_team_players)} players who played for multiple teams"
            )  # noqa: E501
            ...
            for player in multi_team_players:
                ...
                if "MIN" in df.columns:
                    ...
                    for col in numeric_cols:
                        weighted_stats[col] = (
                            player_data[col] * player_data["MIN"]
                        ).sum() / total_minutes
    ...
    def create_game_level_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Estimate games played from minutes (assuming ~30 min per game
        # average)
        if "MIN" in df.columns:
    ...
    def add_target_variable(
        self, df: pd.DataFrame, box_scores: pd.DataFrame = None
    ) -> pd.DataFrame:
        """
        ...
        if box_scores is not None and not box_scores.empty:
            ...
            logger.warning(
                "No box scores available, creating estimated PRA from CTG stats"
            )  # noqa: E501
        ...
        if "Player" in df.columns:
            ...
            if (duplicates > 1).any():
                report["potential_issues"].append(
                    f"Found {(duplicates > 1).sum()} players with multiple entries"  # noqa: E501
                )
    ...
    def prepare_modeling_data(
        self, season: str = "2023 - 24", include_playoffs: bool = False
    ) -> pd.DataFrame:
        """
        ...
        if include_playoffs:
            ...
            if playoff_data:
                ...
                playoff_merged["is_playof"] = 1
                merged_data["is_playof"] = 0
        ...
        logger.info(
            f"Data preparation complete: {quality_report['total_rows']} rows, "
...
def load_multiple_seasons(seasons: List[str], preprocessor: NBADataPreprocessor) -> pd.DataFrame:
    """
    ...
    logger.info(
        f"Loaded {
            len(seasons)} seasons with {
            len(combined_data)} total rows"
    )
...
def main():
    ...
    # Save prepared data
    output_path = (
        "/Users/diyagamah/Documents/nba_props_model/prepared_data_2023_24.csv"  # noqa: E501
    )
