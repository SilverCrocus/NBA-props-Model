...
class CTGRobustScraper:
    def __init__(self):
        ...
        self.data_dir = Path(
            "/Users/diyagamah/Documents/nba_props_model/data/ctg_data_organized"
        )  # noqa: E501
    ...
    def save_progress(self):
        ...
        progress = {
            ...
            "total_expected": len(self.categories)
            * len(self.seasons)
            * len(self.season_types),  # noqa: E501
            "last_updated": datetime.now().isoformat(),
            "completion_rate": f"{len(self.completed_files) * 100 / (len(self.categories) * len(self.seasons) * len(self.season_types)):.1f}%",  # noqa: E501
        }
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "user data directory is already in use" in str(e):
                    print(
                        f"   ⚠️ Chrome profile still locked (attempt {
                    ...
                    if attempt < max_retries - 1:
                        ...
                        self.profile_dir = (
                            Path.home()
                            / f".ctg_chrome_profile_temp_{int(time.time())}"  # noqa: E501
                        )
                        self.profile_dir.mkdir(parents=True, exist_ok=True)
                        self.setup_driver()
                        self.login()
    ...
    def is_file_already_downloaded(self, category, season, season_type):
        ...
        season_folder = (
            "regular_season" if season_type == "Regular Season" else "playoffs"
        )  # noqa: E501
        ...
        return (
            file_path.exists() or (category, season, season_type) in self.completed_files
        )  # noqa: E501
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    print(
                        f"      Temp dir: {
                            self.temp_dir if hasattr(
                                self,
                                'temp_dir') else 'NOT SET'}"
                    )
                    return False
                ...
                try:
                    ...
                    recent_downloads = [
                        f for f in all_downloads if (current_time - f.stat().st_mtime) < 10
                    ]
                    if recent_downloads:
                        print(
                            f"  📥 Found in Downloads folder: {
                                recent_downloads[0].name}"
                        )
                        # Move the file to our temp directory
                        for file in recent_downloads:
                            shutil.move(str(file), self.temp_dir)
                            new_count += 1
                            print("  ✅ Moved from Downloads to temp dir")
                except Exception:  # noqa: F841
                    pass  # Ignore errors checking Downloads

                if new_count <= initial_count:
                    print(f"  ⚠️ No file downloaded to {self.temp_dir}")
                    # Still mark as completed to avoid retrying files that
                    # don't exist
                ...
                if self.organize_downloaded_file(category, season, season_type):
                    ...
                    print(f"  ✅ Downloaded: {category} - {season} - {season_type}")  # noqa: E501
                    return True
                else:
                ...
                if attempt < max_retries - 1:
                    print(f"  ⚠️ Retry {attempt + 1}/{max_retries}: {str(e)[:50]}")  # noqa: E501
                    time.sleep(2)
                else:
    ...
    def organize_downloaded_file(self, category, season, season_type):
        ...
        try:
            ...
            if category_clean.startswith("on_off_"):
                ...
                subfolder_name = category_clean.replace("on_off_", "")
                dest_dir = (
                    self.data_dir / "players" / season / season_folder / "on_of" / subfolder_name
                )
                filename = f"{subfolder_name}.csv"
            else:
                # Regular categories
                dest_dir = (
                    self.data_dir / "players" / season / season_folder / category_clean
                )  # noqa: E501
    ...
    def run_full_scrape(self):
        ...
        needed = [
            ...
            if not self.is_file_already_downloaded(*combo)  # noqa: E501
        ]
        ...
        for i, (category, season, season_type) in enumerate(needed, 1):
            ...
            if i % 10 == 0:
                ...
                print(
                    f"Session files: {self.current_session_files}/{self.max_files_per_session}"
                )  # noqa: E501
                print(f"{'=' * 40}")
