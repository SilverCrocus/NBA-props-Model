...
class CTGTeamScraper:
    ...
    def __init__(self, base_dir="/Users/diyagamah/Documents/nba_props_model/data"):
        ...
        # Use the same Chrome profile as the player scraper (already has CTG
        # login)
    ...
    def load_progress(self):
        ...
        if self.progress_file.exists():
            with open(self.progress_file, "r") as f:
                data = json.load(f)
                logger.info(
                    f"Loaded progress: {len(data.get('completed', []))} files already downloaded"  # noqa: E501
                )
    ...
    def save_progress(self):
        ...
        progress_data = {
            ...
            "total_expected": len(self.teams)
            * len(self.categories),  # 30 teams × 9 categories = 270
    ...
    def navigate_to_team_stats(self, team_id):
        ...
        try:
            ...
            __active_tab = self.driver.find_element(  # noqa: F841
    ...
    def select_category(self, category):
        ...
        try:
            ...
            if not dropdown:
                try:
                    ...
                    for dd in dropdowns:
                        ...
                        if (
                            "Team Efficiency" in options_text or "Shooting" in options_text
                        ):  # noqa: E501
                            dropdown = dd
                            break
                except BaseException:
            ...
            if not dropdown:
                logger.error("Could not find category dropdown on page")
                return False
            ...
            category_map = {
                "Team Efficiency and Four Factors": "Team Efficiency and Four Factors",  # noqa: E501
                "Offense: Shooting Frequency": "Shooting Frequency",
                "Offense: Shooting Accuracy": "Shooting Accuracy",
                "Offense: Play Context: Halfcourt and Putbacks": "Play Context: Halfcourt and Putbacks",  # noqa: E501
                "Offense: Play Context: Transition": "Play Context: Transition",  # noqa: E501
                "Defense: Shooting Frequency": "Shooting Frequency",
                "Defense: Shooting Accuracy": "Shooting Accuracy",
                "Defense: Play Context: Halfcourt and Putbacks": "Play Context: Halfcourt and Putbacks",  # noqa: E501
                "Defense: Play Context: Transition": "Play Context: Transition",  # noqa: E501
            }
            ...
            if "Defense:" in category:
                ...
                for option in options:
                    if option.text == mapped_name and "Defense" not in str(
                ...
                for i, option in enumerate(options):
                    if option.text == mapped_name:
                        occurrence_count += 1
                        if (
                            occurrence_count > offense_count
                        ):  # This is the defense one  # noqa: E501
                ...
                if not defense_found:
                    logger.warning(
                        f"Could not find defense category: {category}, trying by text"
                    )  # noqa: E501
    ...
    def show_playoff_stats(self):
        """Click 'Show Playoff Stats' to include both regular season and playoff data"""  # noqa: E501
        try:
            ...
            logger.info(
                "Note: Could not add playoff stats (may already be shown or unavailable)"
            )  # noqa: E501
            return False

    def download_current_table(self, team_name, category):
        """Download the currently displayed table (contains both regular season and playoff data)"""  # noqa: E501
        try:
            ...
            try:
                download_link = WebDriverWait(self.driver, 10).until(
                    EC.presence_of_element_located(
                        (By.XPATH, "//a[contains(text(), 'Download this table')]")
                    )
                )  # noqa: E501
            except BaseException:
            ...
            if not download_link:
                try:
                    ...
                    for link in links:
                        if (
                            "download" in link.get_attribute("class").lower()
                            or "download" in (link.get_attribute("hre") or "").lower()  # noqa: E501
                        ):
                            download_link = link
                            break
                except BaseException:
            ...
            try:
                download_link.click()
                click_success = True
                logger.info("Clicked download link using regular click")
                ...
                try:
                    ...
                    click_success = True
                    logger.info("Clicked download link using JavaScript")
            ...
            if newest_file:
                ...
                logger.info(
                    f"✅ Downloaded: {team_name} - {category} (includes playoffs)"
                )  # noqa: E501
                return str(final_path)
            else:
                logger.error(
                    f"❌ Download failed: no new file found for {team_name} - {category}"
                )  # noqa: E501
                return None

        except Exception as e:
            logger.error(
                f"Error downloading table for {team_name} - {category}: {str(e)}"
            )  # noqa: E501
    ...
    def scrape_all_teams(self):
        ...
        try:
            __logout_link = self.driver.find_element(  # noqa: F841
        ...
        for team_id, team_name in self.teams.items():
            ...
            for category in self.categories:
                ...
                # Download the table (contains both regular season and playoff
                # data)
                ...
                if file_path:
                    ...
                    logger.info(
                        f"Progress: {total_completed}/{total_files} ({progress_pct:.1f}%)"
                    )  # noqa: E501
        ...
        logger.info(f"\n{'=' * 60}")
        logger.info("✅ TEAM DATA COLLECTION COMPLETE!")
