...
class AdvancedStatsFeatures:
    ...
    def calculate_usage_rate(
        self, df: pd.DataFrame, team_df: Optional[pd.DataFrame] = None
        ...
        Args:
            ...
            team_df: Optional team-level stats (if not provided, estimated from player stats)  # noqa: E501
        ...
        team_stats = (
            df.groupby("GAME_ID")
            .agg({"FGA": "sum", "FTA": "sum", "TOV": "sum", "MIN": "sum"})
        ...
        df["USG_pct"] = np.where(
            ...
            100
            * (player_touches * (df["TEAM_MIN"] / 5))
            / (df["MIN"] * team_touches),  # noqa: E501
            0,
        )
    ...
    def calculate_pace_factor(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        game_stats = df.groupby("GAME_ID").agg(
            {"FGA": "sum", "FTA": "sum", "TOV": "sum", "OREB": "sum", "DREB": "sum"}
        )
    ...
    def add_all_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        new_features = [
            ...
            if any(x in col for x in ["TS_pct", "USG_pct", "per_100", "pace"])
        ]
...
if __name__ == "__main__":
    ...
    print(
        [col for col in result.columns if "TS_pct" in col or "USG" in col or "per_100" in col][:10]
    )  # noqa: E501
