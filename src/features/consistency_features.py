...
class ConsistencyFeatures:
    ...
    def calculate_volatility_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "PRA_std_L10" in df.columns and "PRA_mean_L10" in df.columns:
            df["normalized_volatility"] = df["PRA_std_L10"] / df["PRA_mean_L10"]  # noqa: E501
    ...
    def detect_boom_bust_tendency(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["PRA_avg_L20"] = (
            df.groupby("PLAYER_ID")["PRA"].shift(1).rolling(window=20, min_periods=5).mean()
        )

        # Define boom/bust threshold (1 std deviation)
        df["PRA_std_L20"] = (
            df.groupby("PLAYER_ID")["PRA"].shift(1).rolling(window=20, min_periods=5).std()
        )
        ...
        df["sign_change"] = (
            df["pra_change_sign"] != df.groupby("PLAYER_ID")["pra_change_sign"].shift(1)
        ).astype(int)
        ...
        df.drop(
            ["PRA_shifted", "is_boom", "is_bust", "pra_change", "pra_change_sign", "sign_change"],
            axis=1,
            inplace=True,
            errors="ignore",
        )
    ...
    def calculate_streak_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["PRA_L5_avg"] = (
            df.groupby("PLAYER_ID")["PRA"].shift(1).rolling(window=5, min_periods=1).mean()
        )
        ...
        # Clean up temporary columns
        df.drop(
            ["PRA_L5_avg", "PRA_shifted", "exceeded_avg"], axis=1, inplace=True, errors="ignore"
        )
...
if __name__ == "__main__":
    ...
    print(
        result[result["PLAYER_ID"] == 1][
            ["PRA_CV_L10", "consistency_score_L10", "boom_bust_score"]
        ].tail(1)
    )
    print(
        result[result["PLAYER_ID"] == 2][
            ["PRA_CV_L10", "consistency_score_L10", "boom_bust_score"]
        ].tail(1)
    )
