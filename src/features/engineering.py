...
class NBAFeatureEngineer:
    ...
    def __init__(
        ...
        self.ctg_player_path = self.data_path / "ctg_data_organized" / "players"  # noqa: E501
    ...
    def calculate_usage_rate_ewma(self, df: pd.DataFrame, span: int = 15) -> pd.Series:
        ...
        Formula: USG% = ((FGA + 0.44 * FTA + TOV) * (Tm MP / 5)) / (MP * (Tm FGA + 0.44 * Tm FTA + Tm TOV))  # noqa: E501
        """
    ...
    def calculate_advanced_metrics(self, df: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        if all(col in df.columns for col in ["Usage", "PSA", "AST%", "TOV%"]):
            ...
            advanced_features["per_approximation"] = (
                df["Usage"] * 0.3 + df["PSA"] / 100 * 0.3 + df["AST%"] * 0.2 - df["TOV%"] * 0.2
            )

        # Offensive efficiency composite
        if "PSA" in df.columns and "AST%" in df.columns and "TOV%" in df.columns:  # noqa: E501
            advanced_features["offensive_efficiency"] = (
                df["PSA"] / 100 * 0.5 + df["AST%"] * 0.3 - df["TOV%"] * 0.2
            )
    ...
    def calculate_position_vs_defense(self, df: pd.DataFrame) -> pd.Series:
        ...
        position_defense_modifier = {"Guard": 1.0, "Wing": 1.0, "Big": 1.0}
    ...
    def calculate_rolling_averages(
        self, df: pd.DataFrame, stats: List[str], windows: List[int] = None
    ) -> pd.DataFrame:
        """
        ...
        for stat in stats:
            if stat in df.columns:
                for window in windows:
                    ...
                    rolling_features[col_name] = df.groupby("Player")[stat].transform(  # noqa: E501
    ...
    def calculate_ewma_features(
        self, df: pd.DataFrame, stats: List[str], spans: List[int] = None
    ) -> pd.DataFrame:
        """
        ...
        for stat in stats:
            if stat in df.columns:
                for span in spans:
                    ...
                    ewma_features[col_name] = df.groupby("Player")[stat].transform(  # noqa: E501
                        lambda x: x.ewm(span=span, min_periods=1).mean()
                    )
    ...
    def calculate_volatility(
        self, df: pd.DataFrame, stats: List[str], window: int = 15
    ) -> pd.DataFrame:
        """
    ...
    def calculate_trend_features(
        self, df: pd.DataFrame, stats: List[str], window: int = 10
    ) -> pd.DataFrame:
        """
    ...
    def remove_outliers(
        self, df: pd.DataFrame, cols: List[str], method: str = "iqr", threshold: float = 3.0
    ) -> pd.DataFrame:
        """
    ...
    def normalize_features(
        self, df: pd.DataFrame, method: str = "standard", exclude_cols: List[str] = None
    ) -> pd.DataFrame:
        """
        Normalize features using various methods
        """
        from sklearn.preprocessing import MinMaxScaler, RobustScaler, StandardScaler  # noqa: E501
        ...
        numeric_cols = [
            col for col in df.select_dtypes(include=[np.number]).columns if col not in exclude_cols
        ]
    ...
    def select_features_lasso(
        self, X: pd.DataFrame, y: pd.Series, alpha: float = 0.01
    ) -> List[str]:
        """
        ...
        feature_importance = pd.DataFrame(
            {"feature": X.columns, "coefficient": np.abs(lasso.coef_)}
        )

        selected_features = feature_importance[feature_importance["coefficient"] > 0][  # noqa: E501
            "feature"
        ].tolist()
    ...
    def create_feature_set(
        self, player_data: pd.DataFrame, team_data: pd.DataFrame = None
    ) -> pd.DataFrame:
        """
        ...
        if team_data is not None:
            ...
            try:
                ...
                if (
                    "Opponent" in player_data.columns and "Season" in player_data.columns
                ):  # noqa: E501
                    player_with_opp = opp_feat_engine.get_opponent_features(player_data)
                    matchup_feats = opp_feat_engine.create_matchup_features(
                        player_data, player_with_opp
                    )
        ...
        # Position vs Defense
        features["Pos_vs_De"] = self.calculate_position_vs_defense(player_data)
        ...
        # Trend Features
        trend_features = self.calculate_trend_features(player_data, available_stats[:3], window=10)
        features = pd.concat([features, trend_features], axis=1)

        logger.info(
            f"Feature engineering complete. Created {len(features.columns)} features."
        )  # noqa: E501
...
def main():
    ...
    sample_path = "/Users/diyagamah/Documents/nba_props_model/data/ctg_data_organized/players/2023 - 24/regular_season/offensive_overview/offensive_overview.csv"  # noqa: E501
    ...
    try:
        ...
        if validation_report["zero_variance"]:
            print(
                f"\nZero variance features: {
                    validation_report['zero_variance']}"
            )
        ...
        # Save features
        output_path = (
            "/Users/diyagamah/Documents/nba_props_model/engineered_features.csv"  # noqa: E501
        )
