...
predictions_df = pd.read_csv(
    '/Users/diyagamah/Documents/nba_props_model/data/results/walkforward_predictions_2024 - 25.csv')  # noqa: E501
backtest_df = pd.read_csv(
    '/Users/diyagamah/Documents/nba_props_model/data/results/backtest_walkforward_2024_25.csv')  # noqa: E501
features_df = pd.read_parquet(
    '/Users/diyagamah/Documents/nba_props_model/data/processed/full_2024_25.parquet')  # noqa: E501
...
error_by_range = predictions_df.groupby(
    'pred_range')['abs_error'].agg(['mean', 'std'])
...
ax = axes[0]
history_bins = pd.cut(
    predictions_df['games_in_history'], bins=[
        0, 10, 20, 30, 40, 50, 100], labels=[
...
z = np.polyfit(
    predictions_df['games_in_history'],
...
ax.set_title(
    'Error vs Training Data Size (Sampled)',
    fontsize=14,
    fontweight='bold')
ax.legend()
ax.grid(alpha=0.3)
...
ax.set_title(
    'Players with Highest MAE (min 10 games)',
    fontsize=14,
    fontweight='bold')
ax.grid(alpha=0.3)
...
ax.set_xlabel('Mean Absolute Error', fontsize=12)
ax.set_title(
    'Players with Lowest MAE (min 10 games)',
    fontsize=14,
    fontweight='bold')
ax.grid(alpha=0.3)
...
ax.set_title(
    'Win Rate vs Expected by Edge Bucket',
    fontsize=14,
    fontweight='bold')
ax.legend()
ax.grid(alpha=0.3)
...
# Calibration error
ax = axes[0, 1]
edge_stats['Calibration_Error'] = edge_stats['Win_Rate'] - \
    edge_stats['Expected_Win_Rate']
colors = ['red' if x < 0 else 'green' for x in edge_stats['Calibration_Error']]
ax.bar(
    range(
        len(edge_stats)),
    edge_stats['Calibration_Error'],
    color=colors,
    alpha=0.7)
...
ax.set_title(
    'Calibration Error by Edge Bucket',
    fontsize=14,
    fontweight='bold')
ax.grid(alpha=0.3)
...
ax.scatter(
    backtest_df['edge'],
    backtest_df['bet_won'].astype(int),
    alpha=0.3,
    s=10)
# Add smoothed line
...
binned_win_rate = backtest_df.groupby(
    pd.cut(backtest_df['edge'], bins=20))['bet_won'].mean()
bin_centers = [interval.mid for interval in binned_win_rate.index]
ax.plot(
    bin_centers,
    binned_win_rate.values,
    'r-',
    linewidth=3,
...
if 'pra' in features_df.columns:
    ...
    if correlations:
        ...
        ax.set_title(
            'Top 30 Features by Correlation with PRA',
            fontsize=14,
            fontweight='bold')
