...
These features have 0.80 - 0.85 correlation with PRA and should reduce MAE significantly.  # noqa: E501
"""
...
class CTGFeatureBuilder:
    ...
    def _load_all_seasons(self):
        ...
        for season_dir in season_dirs:
            ...
            offensive_file = (
                season_dir / "regular_season" / "offensive_overview" / "offensive_overview.csv"
            )
            ...
            shooting_file = (
                season_dir / "regular_season" / "shooting_accuracy" / "shooting_accuracy.csv"
            )
            ...
            rebounding_file = (
                season_dir / "regular_season" / "defense_rebounding" / "defense_rebounding.csv"
            )
    ...
    def get_player_ctg_features(self, player_name: str, season: str) -> Dict[str, float]:
        ...
        if season in self.offensive_stats:
            ...
            if len(player_row) > 0:
                ...
                if "Usage" in row:
                    features["CTG_USG"] = (
                        float(row["Usage"]) if pd.notna(row["Usage"]) else features["CTG_USG"]
                    )
                if "PSA" in row:
                    features["CTG_PSA"] = (
                        float(row["PSA"]) if pd.notna(row["PSA"]) else features["CTG_PSA"]
                    )
                if "AST%" in row:
                    features["CTG_AST_PCT"] = (
                        float(row["AST%"]) if pd.notna(row["AST%"]) else features["CTG_AST_PCT"]
                    )
                if "TOV%" in row:
                    features["CTG_TOV_PCT"] = (
                        float(row["TOV%"]) if pd.notna(row["TOV%"]) else features["CTG_TOV_PCT"]
                    )
        ...
        if season in self.shooting_stats:
            ...
            if len(player_row) > 0:
                ...
                if "eFG%" in row:
                    features["CTG_eFG"] = (
                        float(row["eFG%"]) if pd.notna(row["eFG%"]) else features["CTG_eFG"]
                    )
...
if __name__ == "__main__":
    ...
    for player, season in test_cases:
        ...
        print(
            f"  Available: {
                'Yes' if features['CTG_Available'] == 1 else 'No (using league avg)'}"
        )  # noqa: E501
