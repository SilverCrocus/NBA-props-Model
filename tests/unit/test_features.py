...
if "Usage Rank" in merged_df.columns and "PSA Rank" in merged_df.columns:
    temporal_features["Consistency_Score"] = 1 / (
        1 + np.abs(merged_df["Usage Rank"] - merged_df["PSA Rank"]) / 100
    )
...
final_features = pd.concat(
    [merged_df[["Player", "Team"]], core_features, context_features, temporal_features], axis=1
)
...
print(
    f"  Total features: {len(final_features.columns) -
                           2} (excluding Player, Team)"
)
...
print("   - Three-tier architecture implemented")
