...
class TestTemporalLeakagePrevention:
    ...
    def test_lag1_excludes_current_game(self, sample_game_logs):
        ...
        assert (
            matches_current == 0
        ), f"Found {matches_current} cases where lag1 equals current PRA (LEAKAGE!)"  # noqa: E501
    ...
    def test_rolling_avg_excludes_current_game(self, sample_game_logs):
        ...
        assert (
            abs(df.loc[3, "PRA_L3_mean"] - expected_avg) < 0.01
    ...
    def test_no_leakage_across_players(self):
        ...
        assert pd.isna(
            df.loc[3, "PRA_lag1"]
        ), "Player 2's first game should have NaN, not Player 1's data (LEAKAGE!)"  # noqa: E501

        # Player 2's second game should have their own first game
        assert (
            df.loc[4, "PRA_lag1"] == 22
        ), f"Player 2's lag1 should be their own data, got {df.loc[4, 'PRA_lag1']}"  # noqa: E501
    ...
    def test_train_test_split_no_temporal_overlap(self):
        ...
        assert len(overlap) == 0, f"Found {len(overlap)} rows in both train and test (LEAKAGE!)"

    def test_ewma_excludes_current_game(self, sample_game_logs):
        """
        CRITICAL: EWMA (exponentially weighted moving average) must shift.

        EWMA is used for "current form" features - must not include current game.  # noqa: E501
        """
        df = sample_game_logs.copy()
        ...
        assert (
            df.loc[1, "PRA_ewma5"] == 25
        ), f"Expected ewma5=25 (only 1 prev game), got {df.loc[1, 'PRA_ewma5']}"  # noqa: E501
    ...
    def test_opponent_features_no_future_data(self):
        ...
        assert pd.isna(df.loc[0, "opp_avg_PRA"]), "First game vs opponent should have NaN"

        # Third game vs BOS should have avg of first game vs BOS only
        # (excluding current game)
        assert (
            df.loc[2, "opp_avg_PRA"] == 25
...
class TestLeakageDetectionUtility:
    ...
    def test_detect_perfect_correlation(self):
        ...
        df = pd.DataFrame(
            {
                "PRA": [25, 30, 28, 35, 22],
                # Perfect correlation = likely leakage
                "feature": [25, 30, 28, 35, 22],
            }
        )

        correlation = df["PRA"].corr(df["feature"])
        assert abs(correlation) > 0.99, "Should detect perfect correlation (likely leakage)"
    ...
    def test_detect_time_shifted_correlation(self):
        ...
        assert shifted_match == 4, "Should detect that feature matches future PRA (LEAKAGE!)"
...
class TestWalkForwardValidation:
    ...
    def test_walk_forward_maintains_temporal_order(self):
        ...
        for i in range(0, len(df) - train_window - test_window + 1, test_window):
            ...
            overlap = set(train.index) & set(test.index)
            assert len(overlap) == 0, f"Window {i}: Found {len(overlap)} overlapping rows"
